(() => {
  document.addEventListener('DOMContentLoaded', async () => {
    const emptyRowClass = 'clients-table__empty-row';
    const emptyTableTemplate = `<tr class="${emptyRowClass}">
                                  <td colspan="7">
    ...
    const createModal = async (mode, id = null, actionBtnsIconsArr = []) => {
      ...
      if (mode === 'clientDataModal') {
        ...
        submitBtn.addEventListener('click', async (e) => {
          ...
          const toShowRequestStatus = async (response) => {
            ...
            if (status === 422 || status === 404 || String(status).startsWith('5')) {
              ...
              if (id) {
                document.getElementById(data.id).replaceWith(createTableRow(message))
              } else {
                if (container.children[0].className == emptyRowClass) container.innerHTML = '';
                container.append(createTableRow(message));
              }
        ...
        submitBtn.addEventListener('click', async (e) => {
          ...
          if (!container.childElementCount) container.innerHTML = emptyTableTemplate;
          closeModal();
        });
