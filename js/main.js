(() => {
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    const createContact = (data, id, container) => {
      const iconsHTML = {
        'В Контакте': `<svg class="contact-btn__icon" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    const createTableBody = (data, bodyElem) => {
      if (!data.length) {
        bodyElem.querySelector('.clients-table__empty-content').innerHTML = emptyTableTemplate;
      } else {
        bodyElem.innerHTML = '';
      }
